...
table istr year if year>2014, stat(m issanth issbcbs isshltn isskais issoth) nformat(%9.0f) nototal
...
keep if inlist(gra,4,15,16,19) // SF, LA, SD
...
collapse issanth issbcbs isshltn isskais issoth, by(returnhh year gra) fast

g istr = "Inertia" 
replace istr = "No Inertia" if returnhh==0

save "proc/fig1bygra", replace

* Graph by GRA

use "proc/fig1bygra", clear

set scheme plotplain

foreach g in 4 15 16 19 {

graph bar issanth issbcbs isshltn isskais issoth if istr=="No Inertia" & year!=2014 & gra==`g', over(year) stack ///
saving(proc/ino.gph, replace) title("New Households") ytitle("Market Share (%)") legend(title("Insurer") rows(1) pos(6) ///
label(1 "Anthem") label(2 "BCBS") label(3 "HealthNet") label(4 "Kaiser") label(5 "Other"))

graph bar issanth issbcbs isshltn isskais issoth if istr=="Inertia" & year!=2014 & gra==`g', over(year) stack ///
saving(proc/iyes.gph, replace) title("Returning Households") ytitle("Market Share (%)") legend(title("Insurer") rows(1) pos(6) ///
label(1 "Anthem") label(2 "BCBS") label(3 "HealthNet") label(4 "Kaiser") label(5 "Other"))

grc1leg proc/ino.gph proc/iyes.gph
graph export "output\fig1_gra`g'.png", replace
...
g iss5 = 0
replace iss5 = 1 if issanth
replace iss5 = 2 if issbcbs
replace iss5 = 3 if isshltn
replace iss5 = 4 if isskais
replace iss5 = 5 if issoth
lab def iss5 1 "Anthem" 2 "BCBS" 3 "HealthNet" 4 "Kaiser" 5 "Other"
lab val iss5 iss5

g istr = "Inertia" 
replace istr = "No Inertia" if returnhh==0

collapse prem, by(year iss5) fast

* Table

table iss5 year if year>2014, stat(m prem) nformat(%9.0f) nototal
