...
count if autodp==. & idhh==idhh[_n+1] // 4923
...
foreach y of varlist autoplanid autonet autoiss {
...
foreach y of varlist autoprem autoplanchange-autodp { 
	replace `y' = `y'[_n-1] if idhh==idhh[_n-1] & year==2018
	replace `y' = `y'[_n-1] if idhh==idhh[_n-1] & year==2017
	replace `y' = `y'[_n-1] if idhh==idhh[_n-1] & year==2016
	replace `y' = `y'[_n-1] if idhh==idhh[_n-1] & year==2015
...
g lagppre   = p_pre[_n-1]   if idhh == idhh[_n-1] // Is APTC censored if > gross prem?
g lagppost  = p_post[_n-1]  if idhh == idhh[_n-1]
g lagaptc   = aptc[_n-1]    if idhh == idhh[_n-1]
g lagplan   = planid[_n-1]  if idhh == idhh[_n-1]
g lagnet    = netname[_n-1] if idhh == idhh[_n-1]
g lagiss    = issid[_n-1]   if idhh == idhh[_n-1]
g lagmetal  = metal[_n-1]   if idhh == idhh[_n-1]
g lagproduct  = product[_n-1]   if idhh == idhh[_n-1]
g lagcovend = cov_end[_n-1] if idhh == idhh[_n-1]
...
g icatplan = .
replace icatplan = 1 if idhh!=idhh[_n-1]
replace icatplan = 2 if idhh==idhh[_n-1] & netname==netname[_n-1] & metal==metal[_n-1] & product==product[_n-1]
replace icatplan = 3 if idhh==idhh[_n-1] & (netname!=netname[_n-1] | metal!=metal[_n-1] | product!=product[_n-1])
...
g icatnet = .
replace icatnet = 1 if idhh!=idhh[_n-1]
replace icatnet = 2 if idhh==idhh[_n-1] & netname==netname[_n-1] 
replace icatnet = 3 if idhh==idhh[_n-1] & netname!=netname[_n-1] 
...
lab def icatauto 1 "New" 2 "Stayer" 3 "Switcher" 4 "Returner", replace
