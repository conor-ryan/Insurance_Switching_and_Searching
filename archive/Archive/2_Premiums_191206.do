...
* Mean All Other Premiums
gen count = 1
egen opts = sum(count), by(year gra)
egen temp = sum(prem), by(year gra)
g mean_prem_all = (temp - prem) / (opts - 1)
lab var mean_prem_all "Mean of All Other Premiums"
drop temp count opts

* Mean All Other Premiums, Same Metal
gen count = 1
egen opts = sum(count), by(year gra metal)
egen temp = sum(prem), by(year gra metal)
g mean_prem_mtl = (temp - prem) / (opts - 1)
lab var mean_prem_mtl "Mean of Other Premiums, Same Metal"
drop temp count opts
