...
/// IMPUTE APTC FOR NEXT YEAR, CALCULATE CHANGE IN PREMIUM ///
...
g fplnext = fpl[_n+1] if idhh==idhh[_n+1]
corr fpl fplnext // ~1

g agenext = agecurve[_n+1] if idhh==idhh[_n+1]
corr agecurve agenext // .95

g aptcnext = aptc[_n+1] if idhh==idhh[_n+1]
corr aptcnext aptc // .88

* APTC Next Year

g autoaptc = aptc[_n+1] if idhh==idhh[_n+1]
lab var autoaptc "APTC t+1"

drop if year==2018

* Other Covariates

egen plangrafe = group(planid gra year)

egen mtlfe = group(metal)

* Regression

global dem  c.agecurve i.fsize c.fpl c.aptc c.p_post
global plan c.p_post i.mtlfe // netid absorbed
global auto c.autoprem i.autoplanchange i.autonetchange

areg autoaptc $dem if hassub==1, a(plangrafe)
predict autoaptchat1 if hassub==1
corr autoaptc autoaptchat1 if hassub==1
corr autoaptc autoaptchat1 if hassub==1 & autoaptc!=.

areg autoaptc $dem if hassub==0, a(plangrafe)
predict autoaptchat0 if hassub==0
corr autoaptc autoaptchat0 if hassub==0
corr autoaptc autoaptchat0 if hassub==0 & autoaptc!=.

* Predicted APTC

g autoaptchat = autoaptc
replace autoaptchat = autoaptchat1 if hassub==1 & autoaptc==.
replace autoaptchat = autoaptchat0 if hassub==0 & autoaptc==.
