...
* Flag for Individuals choosing Multiple Plans
gen count = 1
egen plan_per_person = sum(count), by(year idind)
gen multplanind = plan_per_person>1
lab var multplanind "Ind Selects >1 Plan"
drop count 
...
* Drop Multiplan Selections
count if multplan==1  
drop if multplan==1       // 234,543 obs; 88,309  HH obs

count if multplanind==1
drop if multplanind==1   // 2,032 

* Individual Age change
egen indpanel    = group(idind)
tsset indpanel year
gen age_diff = d.age
gen ageflag = age_diff!=. & (age_diff<0 | age_diff>2)
egen hhageflag = sum(ageflag), by(idhh)
replace hhageflag = hhageflag>0
drop age_diff ageflag
...
count if hhageflag==1                   

* Implement Filters

count // 5,464,510              
  
drop if multplan==1       // 88,309 (Already Dropped)                     
drop if gra==.            // 17,196                            
drop if fage<18           // 41,686        
drop if csr>0 & hassub==0 // 11,575
drop if hhageflag==1      // 569
