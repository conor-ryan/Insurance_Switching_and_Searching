...
*Still some small unexplained mismatch between auto eligilibility, the actual auto-assigned plan, and the lagged plan choice

gen autoplan = autoplanid==planid
*gen autoplan = netname == autonet & metal == auto_mtl & product==auto_prod
egen hasautoplan = sum(autoplan), by(idhh year)
egen hasiplan = sum(iplan), by(idhh year)
tab hasautoplan autoelig

tab autoelig hasautoplan if autoplan==0 & iplan==1
tab autoplanchange if autoplan==1 & iplan==0

drop autoplan hasiplan hasautoplan
