...
g padj 
table year, c(p50 prem p25 prem p75 prem) f(%9.2f)
...
table year, c(med aptc med agecurve p25 agecurve p75 agecurve)
table year, c(p50 p_pre  p25 p_pre  p75 p_pre)
table year, c(p50 p_post p25 p_post p75 p_post)
...
use idhh year prem pmtlord metal planid icatplan using "proc\ca_enroll_hh_wi", clear
replace prem = prem * 1.024 if year < 2015 // medical CPI
replace prem = prem * 1.026 if year < 2016
replace prem = prem * 1.038 if year < 2017
replace prem = prem * 1.025 if year < 2018
...
g icatdisc = icatplan==4
g hasinert = icatplan==2 | icatplan==3
replace hasinert = hasinert*100
replace icatstay = icatstay*100
egen cohort = min(year), by(idhh)
g same = planid==planid[_n-1]
replace same = . if year==cohort | hasinert==0
replace same = same*100
g mtlmin = pmtlord==1
replace mtlmin = mtlmin*100
table cohort year, c(m same)   f(%9.2f) row
table cohort year, c(m prem)   f(%9.0f) row
table cohort year, c(m mtlmin) f(%9.2f) row
table cohort year, c(m hasinert) f(%9.2f) row
table cohort year if hasinert==100, c(m icatstay) f(%9.2f) row
...
* Appendix Table: Demographics by Inertia

use "proc\ca_enroll_hh_wi", clear
g hasinert = icatplan==2 | icatplan==3
g fam = fsize>1
g fage1 = fage>=18 & fage<=29
g fage2 = fage>=30 & fage<=49
g fage3 = fage>=50 & fage<=64
table hasinert if year>2014, c(m fage1 m fage2 m fage3 m hassub m fam) f(%9.4f)
...
collapse issanth issbcbs isshltn isskais issoth, by(hasinert year) fast
