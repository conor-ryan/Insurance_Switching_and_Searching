...
foreach y of varlist autoplanid autonet autoiss {
	gen `y'_new = `y'[_n-1] if idhh==idhh[_n-1] /*& year==2018 & year[_n-1]==2017*/
	*replace `y' = `y'[_n-1] if idhh==idhh[_n-1] /*& year==2017 & year[_n-1]==2016*/
	*replace `y' = `y'[_n-1] if idhh==idhh[_n-1] /*& year==2016 & year[_n-1]==2015*/
	*replace `y' = `y'[_n-1] if idhh==idhh[_n-1] /*& year==2015 & year[_n-1]==2014*/
	replace `y'_new = "" if year==2014 | idhh!=idhh[_n-1]| year[_n-1]!=year-1
	replace `y' = `y'_new
	drop `y'_new
}

foreach y of varlist autoprem autoplanchange-autodp base_dprem mean_dprem_all mean_dprem_mtl auto_mean_prem_all auto_mean_prem_mtl choiceset_exceed_thresh_95 choiceset_exceed_thresh_98{ 
	gen `y'_new = `y'[_n-1] if idhh==idhh[_n-1] /*& year==2018 & year[_n-1]==2017*/
	*replace `y' = `y'[_n-1] if idhh==idhh[_n-1] /*& year==2017 & year[_n-1]==2016*/
	*replace `y' = `y'[_n-1] if idhh==idhh[_n-1] /*& year==2016 & year[_n-1]==2015*/
	*replace `y' = `y'[_n-1] if idhh==idhh[_n-1] /*& year==2015 & year[_n-1]==2014*/
